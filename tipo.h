...
extern int hashTamanho;            /* Número de posições da tabela hash. */
...
extern int totalUsuarios;          /* Número de utilizadores actualmente guardados. */
...
int alturaNo(No *no); /* Retorna a altura de um nó AVL. */
...
int indiceHash(int codigo); /* Calcula o Í­ndice hash para o código usando o tamanho atual. */
...
void hashRemover(int codigo); /* Remove a entrada da tabela hash pelo código. */
No *hashBuscar(int codigo); /* Pesquisa um produto na tabela hash pelo código. */
...
No *menorNo(No *raiz); /* Retorna o menor nó em uma subarvore AVL. */
No *balancearNo(No *raiz); /* Rebalanceia um nó AVL após inserção ou remoção. */
No *inserirNoAVL(No *raiz, Produto p, No **noAlterado, int *novoProduto); /* Insere ou atualiza um produto na AVL. */
No *removerNoAVL(No *raiz, int codigo, int controlarHash); /* Remove um produto da AVL e atualiza a hash se necessário. */
...
int quantidadeNoCarrinho(ItemCarrinho *carrinhoAtual, int codigo, const char *vendedorEmail); /* Retorna quantas unidades de um produto de um vendedor estão no carrinho. */
...
int existeUsuario(const char *email);        /* Verifica se um email já existe. */
